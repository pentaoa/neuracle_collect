...
def process_image(image_path, file, subdir):
...
def start_collect_thread(npy_index, imagesBuffer, imageNamesBuffer, tasksBuffer, labelsBuffer, csv_dir):
    start_collect(npy_index, imagesBuffer, imageNamesBuffer, tasksBuffer, labelsBuffer, csv_dir)
    imagesBuffer.clear()
    imageNamesBuffer.clear()
    tasksBuffer.clear()
    labelsBuffer.clear()
        ...
        if os.path.isdir(subdir_path):
            for file in sorted(os.listdir(subdir_path)):
                if file.endswith((".jpg", ".png")):
                    ...
                    thread = threading.Thread(target=process_image, args=(image_path, file, subdir))
            ...
            if folder_count % 20 == 0:
                for thread in threads:
                    thread.join()
                start_collect_thread(npy_index, imagesBuffer, imageNamesBuffer, tasksBuffer, labelsBuffer, csv_dir)
                npy_index += 1
                threads = []
